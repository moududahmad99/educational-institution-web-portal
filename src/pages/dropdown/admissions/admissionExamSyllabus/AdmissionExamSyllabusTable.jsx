import AdmissionExamSyllabusRow from "./AdmissionExamSyllabusRow";

const AdmissionExamSyllabusTable = ({ data }) => {
  return (
    <table className="border-collapse w-full vertical-scroll">
      ...
      <tbody>
        {data.map((row, index) => (
          <AdmissionExamSyllabusRow row={row} key={index} />
        ))}
      </tbody>
    </table>
  );
...
export default AdmissionExamSyllabusTable;
