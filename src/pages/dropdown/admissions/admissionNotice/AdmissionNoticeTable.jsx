...
import AdmissionNoticeRow from "./AdmissionNoticeRow";

const AdmissionNoticeTable = ({ data }) => {
  return (
    <table className="border-collapse w-full vertical-scroll">
      ...
      <tbody>
        {data.map((row, index) => (
          <AdmissionNoticeRow row={row} key={index} />
        ))}
      </tbody>
    </table>
  );
...
export default AdmissionNoticeTable;
